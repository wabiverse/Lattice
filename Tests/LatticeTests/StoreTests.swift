...
final class StoreTests: XCTestCase
{
  ...
  func testQueryIteratesOnlyMatchingArchetype()
  {
    ...
    store.query(Position.self, Tag.self).forEach { entity, _, _ in
      seen.append(entity)
    }
  ...
  func testChangedSinceTracksPerRowWrites()
  {
    ...
    store.query(Position.self).forEachChanged(since: checkpoint) { entity, _ in
  ...
  func testQuery3AndQuery4()
  {
    ...
    var seen3 = 0
    store.query(Position.self, Vel.self, C.self).forEachMutatingFirst { _, position, vel, c in
